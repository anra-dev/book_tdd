...
class SendLoginEmailViewTest(TestCase):
    ...
    def test_adds_success_masage(self):
    ...
    def test_cretes_token_associated_with_email(self):
        """тест: создается маркет, связанный с электронной почтой"""
        self.client.post('/accounts/send_login_email', data={
            'email': 'edith@example.com'
        })
    ...
    def test_sends_link_to_login_using_token_uid(self, mock_send_mail):
        """тест: отсылается ссылка на вход в систему, используя uid маркер"""
        self.client.post('/accounts/send_login_email', data={
            'email': 'edith@example.com'
        })
        token = Token.objects.first()
...
class LoginViewTest(TestCase):
    ...
    def test_calls_auth_login_with_user_if_there_is_one(self, mock_auth):
        """тест: вызывается auth.login с пользователем, если такой имеется"""
        response = self.client.get('/accounts/login?token=abcd123')
        self.assertEqual(
    ...
    def test_does_not_login_if_user_is_not_authenticated(self, mock_auth):
        """тест: не региструруется в системе, если пользователь
        не аутетифицирован"""
